    ...
    <div className="min-h-screen bg-background print:bg-white">
      ...
      <main className="max-w-2xl mx-auto px-5 pb-24 space-y-8">
        ...
        {/* ══════════════════════════════════════════════
            ...
            ══════════════════════════════════════════════ */}
        <div
          className="relative -mx-5 px-5 py-10 sm:py-14 overflow-hidden"
          style={{ background: "linear-gradient(160deg, hsl(225, 12%, 6%) 0%, hsl(30, 25%, 8%) 60%, hsl(36, 30%, 10%) 100%)" }}
        >
          {/* Subtle grid texture */}
          <div className="absolute inset-0 opacity-[0.04]"
            style={{ backgroundImage: "repeating-linear-gradient(0deg, hsl(40,20%,60%) 0px, transparent 1px, transparent 32px), repeating-linear-gradient(90deg, hsl(40,20%,60%) 0px, transparent 1px, transparent 32px)" }} />

          <div className="relative max-w-lg mx-auto">
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] mb-3" style={{ color: DARK.accent }}>
              Análise Aprofundada
            </p>
            <h2 className="font-display text-2xl sm:text-3xl font-bold leading-tight mb-2" style={{ color: DARK.text }}>
              O seu perfil tem mais camadas
              <br />
              <span style={{ color: DARK.accent }}>do que este rastreio revela.</span>
            </h2>
            <p className="text-[13px] leading-relaxed mb-6" style={{ color: DARK.muted }}>
              Você respondeu {Object.keys(answers).length} perguntas. O que você leu acima é a superfície. O relatório aprofundado vai a fundo nos seus padrões reais.
            </p>

            {/* What's inside */}
            <div className="space-y-2 mb-8">
              {[
                ["Análise detalhada de cada mecanismo", "Como cada condição opera no seu perfil"],
                ["Cruzamento entre perfis", "Como suas condições interagem e se potencializam"],
                ["Mapa de forças e vulnerabilidades", "O que trabalhar, o que preservar"],
                ["Ações práticas por cluster", "Estratégias concretas adaptadas ao seu perfil"],
                ["Adaptações sugeridas", "Ambiente, rotina e relação com o trabalho"],
              ].map(([title, desc], i) => (
                <div key={i} className="flex gap-2.5 items-start">
                  <span className="shrink-0 mt-1.5 w-1.5 h-1.5 rounded-full" style={{ backgroundColor: DARK.accent, minWidth: 6, minHeight: 6, display: "block" }} />
                  <div>
                    <span className="text-[12px] font-semibold" style={{ color: DARK.text }}>{title}</span>
                    <span className="text-[11px]" style={{ color: DARK.muted }}> — {desc}</span>
                  </div>
                </div>
              ))}
            </div>

            {/* ── BLURRED PREVIEW — extended, rich content ── */}
            <div ref={blurRef} className="relative rounded-xl overflow-hidden mb-8" style={{ border: `1px solid ${DARK.border}` }}>
              <div className="select-none pointer-events-none" style={{ filter: "blur(5px)", userSelect: "none" }}>
                {/* Block 1 */}
                <div className="p-5 border-b" style={{ borderColor: DARK.border }}>
                  <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: DARK.accent }}>Análise de Mecanismo</p>
                  <h4 className="font-display font-semibold mb-2 text-sm" style={{ color: DARK.text }}>TDAH × AH/SD — Tensão Central</h4>
                  <p className="text-[12px] leading-relaxed mb-2" style={{ color: DARK.muted }}>
                    Sua regulação executiva opera com base em circuitos dopaminérgicos que respondem primariamente a estímulos de novidade e recompensa imediata. Quando combinado com alta capacidade cognitiva, este padrão gera uma tensão específica: enquanto sua mente processa em alta velocidade, o sistema executivo falha na iniciação...
                  </p>
                  <p className="text-[12px] leading-relaxed" style={{ color: DARK.muted }}>
                    As estratégias de externalização de funções executivas mais eficazes para o seu perfil incluem sistemas de captura de ideias em tempo real, ambientes modulados por nível de estímulo e rotinas de transição com âncoras sensoriais específicas...
                  </p>
                </div>
                {/* Dark Radar */}
                <div className="p-5 border-b" style={{ borderColor: DARK.border }}>
                  <p className="text-[10px] font-semibold uppercase tracking-widest mb-3" style={{ color: DARK.accent }}>Mapa Aprofundado de Intensidade</p>
                  <DarkRadarChart hypotheses={sortedHypotheses} />
                </div>
                {/* Block 2 */}
                <div className="p-5 border-b" style={{ borderColor: DARK.border }}>
                  <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: DARK.accent }}>Cruzamento</p>
                  <h4 className="font-display font-semibold mb-2 text-sm" style={{ color: DARK.text }}>Trauma × Ansiedade — Ciclo de Reforço</h4>
                  <p className="text-[12px] leading-relaxed mb-2" style={{ color: DARK.muted }}>
                    A coexistência de padrões traumáticos com hipervigilância cognitiva cria um ciclo onde o sistema de antecipação de ameaças alimenta as respostas automáticas de proteção. A intervenção mais eficaz começa pela regulação do sistema nervoso autônomo, antes da reestruturação cognitiva...
                  </p>
                </div>
                {/* Dark Bar Chart */}
                <div className="p-5 border-b" style={{ borderColor: DARK.border }}>
                  <p className="text-[10px] font-semibold uppercase tracking-widest mb-3" style={{ color: DARK.accent }}>Intensidade por Dimensão — Análise Aprofundada</p>
                  <DarkBarChart hypotheses={sortedHypotheses} />
                </div>
                {/* Area Chart decorative */}
                <div className="p-5 border-b" style={{ borderColor: DARK.border }}>
                  <p className="text-[10px] font-semibold uppercase tracking-widest mb-3" style={{ color: DARK.accent }}>Padrão de Oscilação Cognitiva</p>
                  <DarkAreaChart />
                </div>
                {/* Block 3 */}
                <div className="p-5 border-b" style={{ borderColor: DARK.border }}>
                  <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: DARK.accent }}>Curva de Trajetória Personalizada</p>
                  <InterventionCurveChart dark />
                </div>
                {/* Block 4 */}
                <div className="p-5 border-b" style={{ borderColor: DARK.border }}>
                  <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: DARK.accent }}>Mapa de Forças</p>
                  <h4 className="font-display font-semibold mb-2 text-sm" style={{ color: DARK.text }}>Canalizando sua Alta Complexidade Cognitiva</h4>
                  <p className="text-[12px] leading-relaxed mb-2" style={{ color: DARK.muted }}>
                    Sua alta complexidade cognitiva, quando canalizada através de estruturas de suporte executivo, tem capacidade de gerar resultados expressivos em contextos de alta demanda criativa e resolução de problemas não lineares. O relatório identifica 4 contextos específicos onde seu perfil apresenta vantagem comparativa...
                  </p>
                </div>
                {/* Block 5 */}
                <div className="p-5 border-b" style={{ borderColor: DARK.border }}>
                  <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: DARK.accent }}>Estratégias Adaptadas</p>
                  <h4 className="font-display font-semibold mb-2 text-sm" style={{ color: DARK.text }}>Protocolo de Intervenção Personalizado</h4>
                  <p className="text-[12px] leading-relaxed" style={{ color: DARK.muted }}>
                    Com base nos seus padrões específicos, o relatório propõe um protocolo em 3 fases: regulação do sistema nervoso, estruturação executiva e expansão gradual de zona de performance. Cada fase inclui práticas específicas validadas para o seu cluster de perfil...
                  </p>
                </div>
                {/* Block 6 — line pattern */}
                <div className="p-5" style={{ borderColor: DARK.border }}>
                  <p className="text-[10px] font-semibold uppercase tracking-widest mb-3" style={{ color: DARK.accent }}>Trajetória de Expressão de Potencial</p>
                  <DarkAreaChart />
                  <p className="text-[12px] leading-relaxed mt-3" style={{ color: DARK.muted }}>
                    Esta projeção é calculada com base nos seus scores específicos e considera os efeitos cumulativos de intervenção nos 7 clusters avaliados. A divergência entre as curvas aumenta progressivamente após o ponto de inflexão identificado no seu perfil...
                  </p>
                </div>
              </div>
              ...
              <div className="absolute bottom-0 inset-x-0 h-48 pointer-events-none"
                style={{ background: `linear-gradient(to bottom, transparent 0%, ${DARK.bg} 90%)` }} />

              {/* Lock overlay */}
              <div className="absolute inset-x-0 bottom-0 flex flex-col items-center pb-6 pt-12 pointer-events-none">
                <div className="w-8 h-8 rounded-full flex items-center justify-center mb-2"
                  style={{ background: DARK.border }}>
                  <span className="text-base">🔒</span>
                </div>
                <p className="text-[11px] font-medium" style={{ color: DARK.accent }}>Conteúdo bloqueado</p>
              </div>
            </div>

            {/* CTA Button — direct redirect, no form */}
            <button
              onClick={handleUnlockClick}
              className="w-full py-4 rounded-xl font-semibold text-sm tracking-wide transition-all duration-200 hover:opacity-90 hover:scale-[1.01]"
              style={{
                background: "linear-gradient(135deg, hsl(174,62%,29%), hsl(172,55%,20%))",
                color: "hsl(0, 0%, 97%)",
                boxShadow: "0 0 20px hsl(174 55% 39% / 0.25), 0 4px 28px hsl(174,55%,39%/0.4)",
              }}
            >
              Desbloquear Relatório Aprofundado
            </button>
            <p className="text-[10px] text-center mt-2" style={{ color: "hsl(220, 10%, 45%)" }}>
              Acesso imediato · Análise personalizada para o seu perfil
            </p>
          </div>
        </div>
